...
class TestRecipesApi:
    ...
    def test_recipes(self, client, client_admin, client_user1, user1):
        ...
        assert response.status_code == HTTPStatus.CREATED, f'{response.json()}'
        assert Recipe.objects.filter().count() == recipes_before + 1

        assert client_admin.get('/api/recipes/1/').status_code == HTTPStatus.OK
        assert client_user1.get('/api/recipes/1/').status_code == HTTPStatus.OK
        assert client.get('/api/recipes/1/').status_code == HTTPStatus.OK

        assert client_user1.delete(
    ...
    def test_recipes_favorite(self, client, client_admin, client_user1, user1):
        ...
        assert client.delete(url).status_code == HTTPStatus.UNAUTHORIZED
