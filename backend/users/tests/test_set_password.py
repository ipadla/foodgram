...
class TestUsersAuth:
    ...
    def test_user_set_password(
        ...
        data = {'current_password': password_1, 'new_password': password_weak}
        response = client_user1.post(url, data=data)
        assert response.status_code == 400, f'{response.json()}'

        data = {'current_password': password_1, 'new_password': password_2}
        response = client_user1.post(url, data=data)
        assert response.status_code == 204

        url = '/api/auth/token/login/'
        data = {'password': password_2, 'email': user1.email}
